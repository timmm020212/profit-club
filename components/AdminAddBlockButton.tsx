...
export default function AdminAddBlockButton({ masters, services, date, prefillMasterId, prefillStartTime, perms }: Props) {
  const allowed = perms ? perms.schedule : true;
  const [open, setOpen] = useState(false);

  return (
    <>
      <button onClick={() => allowed && setOpen(true)} disabled={!allowed}
